...
1.1 Example: Let's demonstrate that the equation
$$
p^{2}=2
$$
cannot be satisfied by any rational number represented by \(p\).
Supposing there exists such a \(p\), we can express it ...[missing text]...  the following equation establishes \(q^{2}>2\). 
Therefore, \(q\) belongs to \(B\).

Question: Return the full "example" span of this sample if it exists, otherwise output [].
Answer: ['1.1 Example: Let's demonstrate that the equation
$$
p^{2}=2
$$
cannot be satisfied by any rational number represented by \(p\).']
...
Proof Since $B$ is bounded below, $L$ is not empty. Since $L$ consists ...[missing text]... but $\beta$ is not if $\beta>\alpha$. This means that $\alpha=\inf B$.

Question: Return the full "theorem" span of this sample if it exists, otherwise output [].
Answer: ['1.11 Theorem Suppose $S$ is an ordered set with the least-upper-bound property, $B \subset S, B$ is not empty, and $B$ is bounded below. Let $L$ be the set of all lower bounds of $B$. Then
...
Question: Return the full "proof" span of this sample if it exists, otherwise output [].
Answer: ['1.11 Proof Since $B$ is bounded below, $L$ is not empty. Since $L$ consists of exactly those $y \in S$ which satisfy the inequality $y \leq x$ for every $x \in B$, we see that every $x \in B$ is an upper bound of $L$. Thus $L$ is bounded above. Our hypothesis about $S$ implies therefore that $L$ has a supremum in $S$; call it $\alpha$.
...
def get_theorem_field(text: str):
    \"""
    Function to extract the theorem.
    \"""
    pattern = r'(\d+\.\d+ Theorem[\s\S]+?exists in \$S\.\$)'
